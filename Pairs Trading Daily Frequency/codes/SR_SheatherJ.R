...
plot(density(dist5cc,bw=hd5cc),type="l", col="green", main="(a) Top 5 pairs, Committed Capital, after costs", 
     xlab="Sharpe Ratio", ylab="Density", xlim=c(-0.25,1.5),panel.first=grid())
lines(density(mix5cc,bw=hm5cc), type="l", col="blue")
plot(density(dist5fi,bw=hd5fi),type="l", col="green", main="(b) Top 5 pairs, Fully Invested, after costs", 
     xlab="Sharpe Ratio", ylab="Density", xlim=c(-0.25,1.75), ylim=c(0,2.25),panel.first=grid())
lines(density(mix5fi,bw=hm5fi), type="l", col="blue")
plot(density(dist20cc,bw=hd20cc),type="l", col="green", main="(c) Top 20 pairs, Committed Capital, after costs", 
     xlab="Sharpe Ratio", ylab="Density", xlim=c(-0.25,1.75),panel.first=grid())
lines(density(mix20cc,bw=hm20cc), type="l", col="blue")
plot(density(dist20fi,bw=hd20fi),type="l", col="green", main="(d) Top 20 pairs, Fully Invested, after costs", 
     xlab="Sharpe Ratio", ylab="Density", xlim=c(-0.25,2.25), ylim=c(0,1.6),panel.first=grid())
lines(density(mix20fi,bw=hm20fi), type="l", col="blue")
plot(density(dist35cc,bw=hd35cc),type="l", col="green", main="(e) Top 35 pairs, Committed Capital, after costs", 
     xlab="Sharpe Ratio", ylab="Density", xlim=c(-0.25,2.25),ylim=c(0,1.4),panel.first=grid())
lines(density(mix35cc,bw=hm35cc), type="l", col="blue")
plot(density(dist35fi,bw=hd35fi),type="l", col="green", main="(f) Top 35 pairs, Fully Invested, after costs", 
     xlab="Sharpe Ratio", ylab="Density", xlim=c(-0.25,2.25), ylim=c(0,1.6),panel.first=grid())
lines(density(mix35fi,bw=hm35fi), type="l", col="blue")
...
legend(x = "bottom",inset = 0,
       legend = c("Mixed Copula", "Distance"), 
       col=plot_colors, lwd=0.5, cex=1.0, bty="n",horiz = TRUE)
