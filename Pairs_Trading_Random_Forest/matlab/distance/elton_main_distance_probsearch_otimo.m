    ...
    clearvars -except lucros n_simul i_simul pre_stocks; close all; % clc; 
...
while total_prob < limiar && Loop < limiteLoop %criteria to run
    ...
    if Loop == 0
        pre_pairs_number = pairs_number;
        pre_counter = counter;
        
        elton_main_distance();
        stocks = [min_SSE_ro min_SSE_co];
        
        pairs_number = pre_pairs_number;
        counter = pre_counter;
...
%   if valor_otimo == 0
%       valor_otimo = t_SSE;
%       otimo_componentes = [min_SSE_ro min_SSE_co];
...
for p=1:no_pairs
    ...
    elseif (+IPt(j-i+1,min_SSE_ro(p))-IPt(j-i+1,min_SSE_co(p)) > 2*std_limit)
