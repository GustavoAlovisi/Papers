     %% Top 20, C = 0.001 (20 pares, custo de transacao = 20 bps)
    ...
    % Rp_vw_cc (committed capital), Rp_vw_fi (fully invested), Rf (fixed income)
    % RmRf (market excess return)
    STNpairs = zeros(length(Periods)-2,no_pairs); % Sortino Ratio. 
    % Minus 2 semesters (formation period)
    STNw = zeros(length(Periods)-2,4); % Sortino for committed capital,
    % fully invested, fixed income, market excess return

    % To compute some trading statistics (table 2)
    ...
    pairs_number = 0; pair_open = 0; % Count
...
opt_value = 1/1000; %initiate the value to be optimized
Loop = 0; %initiate count
thresh = 0.99; % threshold of the (sum of) probabilities
limitLoop = 1000; % maximum number of loops
total_prob = 0; %sum of top-probabilities
%%%%%%%%%%%%%%
%funcao = @optPort_sharpe; %fitness function
%%%%%%%%%%%%%%%
converg_rate = 0.001; %learning rate
...
while total_prob < thresh && Loop < limitLoop %criteria to run
    
    %Select Stock for the portfolio
    if Loop == 0
       stocks = distance_auxiliar(no_pairs,countLoop);
    else    
        n=1; 
...
SSE = zeros(no_pairs*2,no_pairs*2);
    ...
    % Compute SSE of the assets drawn just above   
    % ----------------------------------------------------
    % Find min SSEs
    % ----------------------------------------------------
...
SSE(ro,co) = max_SSE; % prevent re-selection
 ...
 % disp([min_SSE_ro;min_SSE_co]); % Show the pairs on the screen
 
 %% 2 steps (1. distance, 2. Prob Search)
 
  % Distance Method
%   if opt_value == 0
%       opt_value = t_SSE;
%       opt_components = [min_SSE_ro min_SSE_co];
%       prob_vector(opt_components,2) = max(prob_vector(:,2));
%   end
%       
  
  % Register new optimals
     if t_SSE > opt_value %check new optimal
        opt_value = t_SSE;
        opt_components = stocks;
        prob_vector(opt_components,2) = max(prob_vector(:,2));
     end
     
     %update the matrix of probabilities
      if opt_value>0 
         prob_vector(opt_components,2) = prob_vector(opt_components,2) + converg_rate;
         prob_vector(:,2) = prob_vector(:,2)./sum(prob_vector(:,2));
         prob_vector(:,3) = cumsum(prob_vector(:,2));
         prob_vector(end,3) = 1;
      end
     
     % compute the total probabilities of the top-no_pairs*2
...
disp([min_SSE_ro;min_SSE_co]); % Show the top pairs on the screen
% Obs: Compute portfolio return to fit them in the sequence
    % ----------------------------------------------------
    % Compute returns during the 6 month period
    % ----------------------------------------------------
    count_temp = counter;
...
for p=1:no_pairs
    ...
    if daylag == 0 % w/o one day delay
            ...
            if wi_update == 1
                ...
                % filters to close the pair if it is open
...
%% Include NEFIN risk factors
% (http://www.nefin.com.br/risk_factors.html)
% Similar to Fama-French risk factors (to Brazil)
...
% these functions are in the paths added at the beginning of the code
