    ...
    % caminhos)
    addpath('C:\Program Files\MATLAB\R2017b\toolbox\stats');
    addpath('C:\Users\ITA\Dropbox\PairsTrading_ITA\matlab\distance\Toolbox');
    addpath('C:\Users\ITA\Dropbox\PairsTrading_ITA\data\data_hf')
    addpath('C:\Users\ITA\Dropbox\PairsTrading_ITA\data\data_SP500_1990_2015');
  ...
  %   Pt = exp(cumsum(log(1+Rt)));
    ...
    % Eu considerei apenas as ações que estão na intersecção/união dos 18
    % meses.
    % No final, usei a união (para aumentar um pouco o número de ações na
    % procura do ótimo).
    % ticker2, tickerb não estão nesta base (do brasil). Estas linhas
    % poderiam ser retiradas.

%     [D,ia,ib]=intersect(ticker2(:,countLoop),ticker2(:,countLoop+1));
%     [ic,id]= ismember(D,ticker2(:,countLoop+2));
%     [ie,ig]= ismember(D(ic),tickerb);
%     index_listed= intersect(index_listed,ig');
%     no_listed = size(index_listed,2);

%     [D,ia,ib]=union(ticker2(:,countLoop),ticker2(:,countLoop+1));
%     [ic,id]= ismember(D,ticker2(:,countLoop+2));
%     [ie,ig]= ismember(D(ic),tickerb);
%     index_listed2 = ig';
%     no_listed2 = sum(ie);
...
writetable(table1,'C:\Users\ITA\Dropbox\PairsTrading_ITA\matlab\results\distance\distance_p5c0.xlsx','WriteVariableNames',true,...
        'WriteRowNames',true,'Sheet',1);
    %distance_p5c0 (5 pares, custo C= 0)
writetable(table2,'C:\Users\ITA\Dropbox\PairsTrading_ITA\matlab\results\distance\distance_p5c0.xlsx','WriteVariableNames',true,...
        'WriteRowNames',true,'Sheet',2);
writetable(table3,'C:\Users\ITA\Dropbox\PairsTrading_ITA\matlab\results\distance\distance_p5c0.xlsx','WriteVariableNames',true,...
...
writetable(table4,'C:\Users\ITA\Dropbox\PairsTrading_ITA\matlab\results\distance\distance_p5c0.xlsx','WriteVariableNames',true,...
...
RowNames5 = {'Annualized Mean Return (%)';'Sharpe_Ratio';'Sortino Ratio';...F
    'Annualized Std Dev (%)'; 'Skewness';'Kurtosis'; 'Minimum Daily Return (%)';...
    'Maximum Daily Return (%)'; 'Cumulative Excess Return'};
table5 = table(ds_vw_cc',ds_vw_fi',ds_Rm_Rf',...
    'VariableNames',VariableNames5,'RowNames',RowNames4);

writetable(table5,'C:\Users\ITA\Dropbox\PairsTrading_ITA\matlab\results\distance\distance_p5c0.xlsx','WriteVariableNames',true,...
        'WriteRowNames',true,'Sheet',5);
  
% saving the .mat file
save('C:\Users\ITA\Dropbox\PairsTrading_ITA\matlab\results\distance\mat_files\distance_p5c0.mat');