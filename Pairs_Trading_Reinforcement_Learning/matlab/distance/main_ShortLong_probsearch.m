    ...
    no_pairs = 20;
    port = 2; % number of elements in the short/long portfolio
...
while total_prob < limiar && Loop < limiteLoop %criteria to run
    ...
    % Combinations
    % ----------------------------------------------------
    comb = nchoosek(stocks,2);
    seq = [0,2,5,9,14,20,27,35,44,54,65,77,90,104,119,135,152,170,189,209,...
        230,252,275,299,324,350,377,405,434,464,495,527,560,594,629,665,702,...
        740];
    ...
    SSE = zeros(no_pairs*2-2,size(comb,1));
        for j=1:(no_pairs*2-2) % calculate SSE for all combinations
            for k=(no_pairs*2*j - seq(j)):size(comb,1) % can prob. be speeded up by better programming
...
%                       IPt(1:Twe_mo,index_listed(k))));
                 % Elo Rating, Mannhatan Distance
                  SSE(j,k) = sum(abs(0.5-1./( 1+ exp(-(IPt(1:Twe_mo,stocks(j))-...
                     IPt(1:Twe_mo,comb(k,:))*ones(port,1)/port )))));
            end
        end
...
for p=1:no_pairs
        ...
        %std_limit = std(1./(1+exp(IPt(1:Twe_mo,min_SSE_ro(p))-IPt(1:+Twe_mo,min_SSE_co(p)))));
         std_limit = std(1./(1+exp(IPt(1:Twe_mo,min_SSE_ro(p))-IPt(1:+Twe_mo,comb(min_SSE_co(p),:))*ones(port,1)/port)));
    ...
    if daylag == 0 % w/o one day delay
            if pairs_opened == -1 % pairs opened: long 1st, short 2nd stock
               ...
               Rpair(counter,p) = +Rt(j,min_SSE_ro(p)) .* wi(1)- Rt(j,comb(min_SSE_co(p),:))*ones(port,1)/port .* wi(2);
            ...
            if wi_update == 1 % The weight of each asset in the pair is updated. 
                            ...
                            wi(2)= wi(2)*(1+Rt(j,comb(min_SSE_co(p),:))*ones(port,1)/port);
            end
            ...
            if wi_update == 1
                ...
                if ((pairs_opened == +1 && (IPt(j-i+1,min_SSE_ro(p))-IPt(j-i+1,comb(min_SSE_co(p),:))*ones(port,1)/port) <= 0)...
                   || Rcum_ret(end,1) <= Stop_loss || (Rcum_ret(end,1) >= Stop_gain) ...
                   || (pairs_opened == -1 && (IPt(j-i+1,min_SSE_ro(p))-IPt(j-i+1,comb(min_SSE_co(p),:))*ones(port,1)/port) >= 0))
                ...
                elseif (+IPt(j-i+1,min_SSE_ro(p))-IPt(j-i+1,comb(min_SSE_co(p),:))*ones(port,1)/port > 2.0*std_limit) 
                if pairs_opened == 0 % record dev (and time) at open
                    Rcum = zeros(Six_mo,1);
                    counter_ret = 1;
                    Av_price_dev(counter,p) = 2*(+IPt(j-i+1,min_SSE_ro(p))- IPt(j-i+1,comb(min_SSE_co(p),:))*ones(port,1)/port)...
                                              /(IPt(j-i+1,min_SSE_ro(p))+ IPt(j-i+1,comb(min_SSE_co(p),:))*ones(port,1)/port);
    ...
    if pairs_opened == -1 % pairs opened: long 1st, short 2nd stock
        Rpair(counter,p) = (+Rt(j,min_SSE_ro(p)) * wi(1) - Rt(j,comb(min_SSE_co(p),:))*ones(port,1)/port* wi(2)) - (lag==2)*trading_cost;
        Rcum (counter_ret,1) = Rpair(counter,p); 
        counter_ret = counter_ret + 1;
        Rcum_ret = cumprod(1+Rcum);
    if wi_update == 1
