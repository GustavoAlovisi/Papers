...
opt_cop2 = zeros(Years * 2 -2,no_pairs);
% ----------------------------------------------------
...
no_listed2 = no_listed; % Lines 87-91 are used in Performance of Pairs 
                        % Trading on the S&P 500: Distance and Mixed Copula
                        % models
...
%  
LL_ecdf = [LL1_ecdf; LL2_ecdf; LL5_ecdf; LL6_ecdf; LL8_ecdf; LLCFG_ecdf];
...
for io=1:no_pairs
opt_cop2(big_loop,io) = find(LL_ecdf(:,io)==min(LL_ecdf(:,io)));
end
...
ddu_t = ddu_t - 0.5; ddu_tc = ddu_tc - 0.5; ddu_tf = ddu_tf - 0.5; ddu_tg = ddu_tg - 0.5;
ddu_tm = ddu_tm - 0.5; ddu_tn = ddu_tn - 0.5; ddu_tt = ddu_tt - 0.5;
ddv_t = ddv_t - 0.5; ddv_tc = ddv_tc - 0.5; ddv_tf = ddv_tf - 0.5; ddv_tg = ddv_tg - 0.5;
ddv_tm = ddv_tm - 0.5; ddv_tn = ddv_tn - 0.5; ddv_tt = ddv_tt - 0.5;

save('C:\Users\fsabino\Dropbox\pairs_trading_hf\dados\ddvt.mat');
