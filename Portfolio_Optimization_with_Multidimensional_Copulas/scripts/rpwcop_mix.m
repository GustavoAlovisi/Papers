...
% % 
% % 
% res_aux = res(1010:6423,:); sig_aux = sig(1010:6423,:);
% 
% for i = 1:(L-T); 
% 
%     disp(['i ',num2str(i)]);
...
% 
%  s1(p,1) = sqrt(w1(i)+ a1(i)*(res_aux(i,1)).^2 + b1(i)*sig_aux(i,1).^2);
%  s2(p,1) = sqrt(w2(i)+ a2(i)*(res_aux(i,2)).^2 + b2(i)*sig_aux(i,2).^2);
%  s3(p,1) = sqrt(w3(i)+ a3(i)*(res_aux(i,3)).^2 + b3(i)*sig_aux(i,3).^2);
%  s4(p,1) = sqrt(w4(i)+ a4(i)*(res_aux(i,4)).^2 + b4(i)*sig_aux(i,4).^2);
%  s5(p,1) = sqrt(w5(i)+ a5(i)*(res_aux(i,5)).^2 + b5(i)*sig_aux(i,5).^2);
%  s6(p,1) = sqrt(w6(i)+ a6(i)*(res_aux(i,6)).^2 + b6(i)*sig_aux(i,6).^2);
%  s7(p,1) = sqrt(w7(i)+ a7(i)*(res_aux(i,7)).^2 + b7(i)*sig_aux(i,7).^2);
%  s8(p,1) = sqrt(w8(i)+ a8(i)*(res_aux(i,8)).^2 + b8(i)*sig_aux(i,8).^2);
%  s9(p,1) = sqrt(w9(i)+ a9(i)*(res_aux(i,9)).^2 + b9(i)*sig_aux(i,9).^2);
%  s10(p,1) = sqrt(w10(i)+ a10(i)*(res_aux(i,10)).^2 + b10(i)*sig_aux(i,10).^2);
%  s11(p,1) = sqrt(w11(i)+ a11(i)*(res_aux(i,11)).^2 + b11(i)*sig_aux(i,11).^2);
%  s12(p,1) = sqrt(w12(i)+ a12(i)*(res_aux(i,12)).^2 + b12(i)*sig_aux(i,12).^2);
%  s13(p,1) = sqrt(w13(i)+ a13(i)*(res_aux(i,13)).^2 + b13(i)*sig_aux(i,13).^2);
%  s14(p,1) = sqrt(w14(i)+ a14(i)*(res_aux(i,14)).^2 + b14(i)*sig_aux(i,14).^2);
%  s15(p,1) = sqrt(w15(i)+ a15(i)*(res_aux(i,15)).^2 + b15(i)*sig_aux(i,15).^2);
%  s16(p,1) = sqrt(w16(i)+ a16(i)*(res_aux(i,16)).^2 + b16(i)*sig_aux(i,16).^2);
%  s17(p,1) = sqrt(w17(i)+ a17(i)*(res_aux(i,17)).^2 + b17(i)*sig_aux(i,17).^2);
%  s18(p,1) = sqrt(w18(i)+ a18(i)*(res_aux(i,18)).^2 + b18(i)*sig_aux(i,18).^2);
%  s19(p,1) = sqrt(w19(i)+ a19(i)*(res_aux(i,19)).^2 + b19(i)*sig_aux(i,19).^2);
%  s20(p,1) = sqrt(w20(i)+ a20(i)*(res_aux(i,20)).^2 + b20(i)*sig_aux(i,20).^2);
%  s21(p,1) = sqrt(w21(i)+ a21(i)*(res_aux(i,21)).^2 + b21(i)*sig_aux(i,21).^2);
%  s22(p,1) = sqrt(w22(i)+ a22(i)*(res_aux(i,22)).^2 + b22(i)*sig_aux(i,22).^2);
%  s23(p,1) = sqrt(w23(i)+ a23(i)*(res_aux(i,23)).^2 + b23(i)*sig_aux(i,23).^2);
%  s24(p,1) = sqrt(w24(i)+ a24(i)*(res_aux(i,24)).^2 + b24(i)*sig_aux(i,24).^2);
%  s25(p,1) = sqrt(w25(i)+ a25(i)*(res_aux(i,25)).^2 + b25(i)*sig_aux(i,25).^2);
%  s26(p,1) = sqrt(w26(i)+ a26(i)*(res_aux(i,26)).^2 + b26(i)*sig_aux(i,26).^2);
%  s27(p,1) = sqrt(w27(i)+ a27(i)*(res_aux(i,27)).^2 + b27(i)*sig_aux(i,27).^2);
%  s28(p,1) = sqrt(w28(i)+ a28(i)*(res_aux(i,28)).^2 + b28(i)*sig_aux(i,28).^2);
%  s29(p,1) = sqrt(w29(i)+ a29(i)*(res_aux(i,29)).^2 + b29(i)*sig_aux(i,29).^2);
%  s30(p,1) = sqrt(w30(i)+ a30(i)*(res_aux(i,30)).^2 + b30(i)*sig_aux(i,30).^2);
%  s31(p,1) = sqrt(w31(i)+ a31(i)*(res_aux(i,31)).^2 + b31(i)*sig_aux(i,31).^2);
%  s32(p,1) = sqrt(w32(i)+ a32(i)*(res_aux(i,32)).^2 + b32(i)*sig_aux(i,32).^2);
%  s33(p,1) = sqrt(w33(i)+ a33(i)*(res_aux(i,33)).^2 + b33(i)*sig_aux(i,33).^2);
%  s34(p,1) = sqrt(w34(i)+ a34(i)*(res_aux(i,34)).^2 + b34(i)*sig_aux(i,34).^2);
%  s35(p,1) = sqrt(w35(i)+ a35(i)*(res_aux(i,35)).^2 + b35(i)*sig_aux(i,35).^2);
%  s36(p,1) = sqrt(w36(i)+ a36(i)*(res_aux(i,36)).^2 + b36(i)*sig_aux(i,36).^2);
%  s37(p,1) = sqrt(w37(i)+ a37(i)*(res_aux(i,37)).^2 + b37(i)*sig_aux(i,37).^2);
%  s38(p,1) = sqrt(w38(i)+ a38(i)*(res_aux(i,38)).^2 + b38(i)*sig_aux(i,38).^2);
%  s39(p,1) = sqrt(w39(i)+ a39(i)*(res_aux(i,39)).^2 + b39(i)*sig_aux(i,39).^2);
%  s40(p,1) = sqrt(w40(i)+ a40(i)*(res_aux(i,40)).^2 + b40(i)*sig_aux(i,40).^2);
%  s41(p,1) = sqrt(w41(i)+ a41(i)*(res_aux(i,41)).^2 + b41(i)*sig_aux(i,41).^2);
%  s42(p,1) = sqrt(w42(i)+ a42(i)*(res_aux(i,42)).^2 + b42(i)*sig_aux(i,42).^2);
%  s43(p,1) = sqrt(w43(i)+ a43(i)*(res_aux(i,43)).^2 + b43(i)*sig_aux(i,43).^2);
%  s44(p,1) = sqrt(w44(i)+ a44(i)*(res_aux(i,44)).^2 + b44(i)*sig_aux(i,44).^2);
%  s45(p,1) = sqrt(w45(i)+ a45(i)*(res_aux(i,45)).^2 + b45(i)*sig_aux(i,45).^2);
%  s46(p,1) = sqrt(w46(i)+ a46(i)*(res_aux(i,46)).^2 + b46(i)*sig_aux(i,46).^2);
%  s47(p,1) = sqrt(w47(i)+ a47(i)*(res_aux(i,47)).^2 + b47(i)*sig_aux(i,47).^2);
%  s48(p,1) = sqrt(w48(i)+ a48(i)*(res_aux(i,48)).^2 + b48(i)*sig_aux(i,48).^2);
%  s49(p,1) = sqrt(w49(i)+ a49(i)*(res_aux(i,49)).^2 + b49(i)*sig_aux(i,49).^2);
%  s50(p,1) = sqrt(w50(i)+ a50(i)*(res_aux(i,50)).^2 + b50(i)*sig_aux(i,50).^2);
%         %%
...
pweights = zeros(L-T,n); VaR = zeros(L-T,1); CVaR = zeros(L-T,1); wm = zeros(L-T,n);
...
ret_mix(i) = wm(i,:)*Rt50(i+2,:)' - Rf(T+i);
ret_mix_gross(i) = wm(i,:)*Rt50(i+2,:)';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Equal Weight Strategy %%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ret_1N(i) = Rt50(i+2,:)*one_over_n - Rf(T+i); 
ret_1N_gross(i) = Rt50(i+2,:)*one_over_n;
%%%%%%%%%%%%%%%%%%%%
%%%Benchmark S&P500 %
%%%%%%%%%%%%%%%%%%%%%
ret_SP500(i) = Rm(T+i)-Rf(T+i); 
ret_SP500_gross(i) = Rm(T+i,1);
end

Sortino_ret_mix= sqrt(252)*sortinoratio(ret_mix,0);
Sortino_ret_mix_gross= sqrt(252)*sortinoratio(ret_mix_gross,0);

%--------------------------------------------------------------------------
% Compute statistics
%--------------------------------------------------------------------------
[return_mix, std_mix, SR_mix, turnover_mix] =...
stats(Rt50(2:end,:),L,T,ret_mix,wm'); 
[return_1N, std_1N, SR_1N, turnover_1N] =...
stats(Rt50(2:end,:),L,T,ret_1N,repmat(one_over_n,1,L-T)); 
[return_mix_gross, std_mix_gross, SR_mix_gross, turnover_mix_gross] =...
stats(Rt50(2:end,:),L,T,ret_mix_gross,wm');
[return_1N_gross, std_1N_gross, SR_1N_gross, turnover_1N_gross] =...
stats(Rt50(2:end,:),L,T,ret_1N_gross,repmat(one_over_n,1,L-T));
...
for k=int_w(j)+1:int_w(j+1)
    ...
    MaxDD_mix(k,:)= cumprod(1+ret_mix(:,k:per6(j)+k-1,:));
    MaxDD_mix_gross(k,:)= cumprod(1+ret_mix_gross(:,k:per6(j)+k-1,:));

end
end

for l=1:5413
MaxDD2_mix(l)= (1+ret_mix(:,l))*(1+ret_mix(:,l+1));
MaxDD2_mix_gross(l)= (1+ret_mix_gross(:,l))*(1+ret_mix_gross(:,l+1));
end



Bstar = opt_block_length([ret_mix', ret_SP500']);
SBstar = Bstar(1,:);
% 
% %--------------------------------------------------------------------------
% % Testa diferença nos índice de Sharpe e no desvio padrão entre mix
% % versus o S&P500
% %--------------------------------------------------------------------------
% 
[pvalSR_mix, pvalMEAN_mix] = dgnu_StatisticsPolitisRomano_v5(ret_mix', ret_SP500',mean(SBstar));
if pvalSR_mix<0.10 && SR_mix>(mean(ret_SP500)/std(ret_SP500));
display('Minimum variance IS statistically different (greater) from SP500')
elseif pvalSR_mix<0.10 && SR_mix<(mean(ret_SP500)/std(ret_SP500));
display('Minimum variance IS statistically different (lesser) from SP500')
elseif pvalSR_mix>0.10
display('Minimum variance IS statistically equal to SP500')
end
if pvalMEAN_mix<0.10 && (mean(ret_mix)<mean(ret_SP500));
display('Standard deviation of minimum variance statistically different (lesser) from SP500')
elseif pvalMEAN_mix<0.10 && (mean(ret_mix)>mean(ret_SP500));
display('Standard deviation of minimum variance statistically different (greater) from SP500')
elseif pvalMEAN_mix>0.10
...
figure,
plot(xData,[cumprod(1+ret_mix)'-1 cumprod(1+ret_1N)'-1 cumprod(1+ret_SP500)'-1]),
legend('mix' , 'Equal Weights', 'SP500')
...
breakeven_mix = mean(ret_mix)*100/(turnover_mix*(1+mean(ret_mix)));
...
Table = [(1+return_mix)^252 std_mix*sqrt(252) SR_mix*sqrt(252) turnover_mix breakeven_mix;
...
Table_acum = [cumprod(1+ret_mix)'-1 cumprod(1+ret_1N)'-1 cumprod(1+ret_SP500)'-1];

save('C:\Users\Fernando\Desktop\paper3\mix_sse\rwpcop_mix.mat')